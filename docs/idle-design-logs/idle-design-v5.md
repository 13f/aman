...
>   2 个 P3 边界条件（序列化残留、depth 不保证重置）——全部修复。R4 的核心特征：运行时行为与设计文档的忠实度审计。
...
impl IdleCoordination {
    ...
    /// 同时设置 real_event_seen 标志，通知 IdleDetector 强制重置 depth。
...
///   （Dispatcher 在 cancel_idle_workflows 中设置），强制重置 depth。
#[async_trait]
...
## 10. Known Risks & Mitigations (R4 Updated)

| 风险 | 来源 | 严重性 | 应对策略 |
|------|------|--------|---------|
| **Pipeline 空闲状态打断矩阵描述不准确** | **R4-1 P2** | **已修复** | §4.4 表格新增"可被真实事件打断？"列；Boredom/Daze/Waiting 标注为 Pipeline 同步执行 |
