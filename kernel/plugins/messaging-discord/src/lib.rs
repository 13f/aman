...
#![forbid(unsafe_code)]
#![doc = "Discord messaging channel integration for aman."]
...
use semver::Version;
use sender::DiscordSender;
use source::DiscordSource;
use std::sync::Arc;
...
impl Plugin for DiscordPlugin {
    ...
    fn event_sources(&self) -> Vec<Arc<dyn EventSource>> {
        ...
        let source = DiscordSource::new(source_id, &config.bot_token);
...
mod tests {
    ...
    fn event_sources_enabled_registers_sender_and_returns_source() {
        ...
        assert_eq!(sources[0].id(), "chat:discord:bot");
        assert_eq!(registry.len(), 1);
        assert!(registry.get("chat:discord:bot").is_some());
    }
}
