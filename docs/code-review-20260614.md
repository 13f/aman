...
- **范围**：`agent_runtime.rs` / `http.rs` / `agent_harness.rs` 均已新增本地 helper 单元测试，但三个文件的集成测试（完整 runtime → HTTP → harness 链路）仍未覆盖。
