...
```
                    ...
                    └──────┬───────┘
                           │ LLM 回复文本
                    ┌──────▼───────┐
                    │  回复输出     │  → 通过 Event Bus 返回给页面（见下方输出路径说明）
                    └──────────────┘
```

**回复输出路径（取决于 Provider 是否支持流式）：**
- **流式**：LLM Provider Tool → Event Bus（发布 `LLM_STREAM_START/CHUNK/TOOL_CALL/TOOL_RESULT/DONE` 系列事件）→ 页面订阅并逐步渲染
- **非流式**：LLM Provider Tool → LLM Skill（等待完整回复）→ Event Bus（发布 `LLM_REPLY_READY` 事件）→ 页面监听渲染
- 所有回复输出均通过 Event Bus，不采用直接 IPC 或页面缓存写入
...
| session_idle_timeout | 用户空闲超时（IDLE/ACTIVE 态 → TIMEOUT） | 300s |
| error_auto_close_after | ERROR 态自动归入 CLOSED 的超时（给用户更多时间 retry） | 600s |
| llm_stream_timeout | LLM Provider 流式响应的最大静默时间（PROCESSING → TIMEOUT） | 120s |
| max_message_length_chars | 单条消息最大 Unicode 代码点数量。4096 字符在最坏情况（全中文字符）下 ≈ 12KB UTF-8，确保下游组件（InputSanitizer, LLM Provider API）可处理 | 4096 chars |
...
OutputValidator 自身可能因崩溃或超时而失效。文档定义其为 **fail_closed** 组件：

> 注意：所有枚举值配置字符串使用 **snake_case** 命名规范（如 `fail_closed`、`replace_token`、`warn_only`），以保持全文档一致性并在 Rust/Python 配置解析中无歧义。
...
失效策略:
  ...
  - 故障告警
    - Validator 每次 fail_closed 触发审计告警（Alert severity: critical）
...
- WAL 保留策略：事件消费确认后删除，或 TTL 7 天自动清理（与 processed_events TTL 一致，防止去重窗口小于恢复窗口）
- **WAL 磁盘配额**：`wal_max_size: 500MB`（默认），超过后：
  - 选项 A：停止写入 WAL（仅内存处理，降级为不保证 WAL 恢复，即 best-effort）
  - 选项 B：滚动删除最早的事件（即使未消费，牺牲可恢复性保系统存活）
- **可观测性**：`wal_disk_usage_percent` 指标，>80% 时告警。WAL 接近配额时，应主动提升 Event Bus 背压等级
...
|--------|------|--------|--------|--------|------|
| `channel_type` | enum | `terminal` | §3 | Source | 渠道类型: terminal / websocket / tauri_desktop |
| `listen_addr` | string | `127.0.0.1:0` | §3 | Source | WebSocket 监听地址 |
| `session_idle_timeout` | duration | `300s` | §3, §6 | Session | IDLE/ACTIVE 态进入 TIMEOUT 的空闲超时 |
| `error_auto_close_after` | duration | `600s` | §3, §6 | Session | ERROR 态自动归入 CLOSED 的超时 |
| `llm_stream_timeout` | duration | `120s` | §3, §6 | Session | LLM 流式响应的最大静默时间（PROCESSING → TIMEOUT） |
| `max_message_length_chars` | int | `4096` | §3 | Source | 单条消息最大 Unicode 代码点数量 |
| `rate_limit` | int | `10` | §3 | User | 用户消息频率限制（条/分钟） |
| `queue_depth_per_session` | int | `10` | §4 | Session | 会话级等待队列最大深度 |
| `queue_overflow_strategy` | enum | `drop` | §4 | Session | 队列溢出策略: drop / preempt_oldest |
| `client_side_prompt_check` | enum | `warn_only` | §9.1, §14.8 | Global | 客户端预检策略: warn_only / block |
| `OutputValidator.timeout` | duration | `2s` | §9.2 | Global | 单次输出验证超时 |
| `WAL.max_size` | bytes | `500MB` | §10.1 | Global | WAL 磁盘配额上限 |
| `WAL.retention_ttl` | duration | `7d` | §10.1 | Global | WAL 保留 TTL（与 processed_events 一致） |
| `dedup.retention_ttl` | duration | `7d` | §10.1 | Global | processed_events 去重集合的 TTL |
| `session.close_timeout` | duration | `5s` | §14.11 | Session | /session close 等待 in-flight Tool 完成的超时 |
| `trim.threshold_ratio` | float | `0.8` | §14.9 | Session | 历史裁剪触发阈值（context_window 的百分比） |
| `trim.minimum_messages` | int | `5` | §14.9 | Session | 裁剪后至少保留的消息数（建议 5-20） |
| `trim.unit_pairs` | bool | `true` | §14.9 | Session | 以 user+assistant 一对为单位裁剪 |
| `queue.wait_stall_threshold` | duration | `60s` | §4 | Session | 队列等待超时检测阈值（触发 QUEUE_STALLED） |

命名风格：所有配置键使用 `snake_case`。层级关系使用 `.` 分隔（如 `WAL.max_size`）。
...
| 属性 | 值 | 配置参数 |
|------|-----|---------|
| 触发条件 | LLM 上下文总 token 数 > context_window × 0.8（80% 阈值，可配置） | `trim_threshold_ratio: 0.8` |
| 裁剪单位 | 从最早的 `{role: user}` 消息开始，每次移除 2 条（user + assistant 为一对） | `trim_unit_pairs: true` |
| 保留最低数量 | 至少保留最近 5-20 条消息（取决于 SOUL system prompt 长度 + 模型 context window） | `trim_minimum_messages: 5`（建议 5-20） |
...
| 标签状态 | 对应 Workflow 状态 | 含义 | 页面表现 |
|---------|-------------------|------|---------|
| 活跃 (active) | ACTIVE / IDLE + 页面焦点 | 用户正在与标签交互 | 高亮显示 |
| 等待回复 (waiting) | PROCESSING | 用户已发送消息，Agent 正在处理 | 标签上显示旋转指示器 |
| 空闲 (idle) | IDLE（无未读事件） | 会话打开但无活动 | 正常显示 |
| 收到新消息 (notified) | IDLE + 后台标签收到事件 | 后台会话收到新回复（用户在其他标签） | 标签上显示小红点/计数器 |
| 已超时 (timeout) | TIMEOUT | 会话空闲超时 | 标签灰显，提示用户恢复或关闭 |
| 错误 (error) | ERROR | 会话处理过程中出错 | 标签红色标记 |
