...
fn push_chat_event(
    ...
    let resolution = sticky_router.resolve(PlatformKind::Discord, chat_id, text);
    let session_id = make_session_id(PlatformKind::Discord, chat_id);
...
impl DiscordSource {
    #[must_use]
    pub fn new(id: impl Into<String>, bot_token: impl Into<String>) -> Self {
...
impl EventSource for DiscordSource {
    ...
    async fn init(&mut self, _ctx: SourceContext) -> AmanResult<()> {
        ...
        let _sticky_router = self
            .sticky_router
            .clone()
            .ok_or_else(|| kernel::Error::config_invalid("DiscordSource missing sticky_router"))?;
        let _chat_session_store = self
            .chat_session_store
            .clone()
            .ok_or_else(|| kernel::Error::config_invalid("DiscordSource missing chat_session_store"))?;
        ...
        self.task = Some(tokio::spawn(async move {
            // Discord Gateway / WebSocket listener loop.
            // Full integration: use serenity's Client with GatewayIntents
            // to receive MessageCreate events, then call push_chat_event().
            tracing::info!(%source_id, "discord source: listener started");
            ...
            tracing::info!(%source_id, "discord source: stopped");
        }));
