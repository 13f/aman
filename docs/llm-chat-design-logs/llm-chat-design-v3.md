...
| cost_aware | 根据请求复杂度选择: 简单请求用低成本模型，复杂请求用高成本模型。预算计数器使用原子操作（读取→决策→扣减三步为原子事务），防止并发会话的预算超支 |
| round_robin | 依次轮换 |
...
| 分类 | 包含命令 | 队列行为 |
|------|---------|---------|
| **非 LLM 命令** | `/session list`, `/session rename`, `/help`, `/debug`, `/export`, `/soul show` | **跳过队列**，立即执行 |
| **LLM 依赖命令** | `/retry`, `/edit`, `/session new`, `/retry --full` | **进入队列**，与 MESSAGE_RECEIVED 共用 FIFO 顺序 |
| **中断命令** | `/stop` | **特殊处理**：无需排队，直接注入 PROCESSING 态的取消信号 |
