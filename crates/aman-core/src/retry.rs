use crate::error::{AmanError, AmanResult};
...
impl RetryBackoff {
    ...
    pub fn first_delay(&self) -> Option<Duration> {
        match self {
            ...
            Self::Sequence(delays) => delays.first().map(|delay_ms| Duration::from_millis(*delay_ms)),
        }
    }
}
...
impl FromStr for RetryBackoff {
    ...
    fn from_str(value: &str) -> AmanResult<Self> {
        ...
        if let Some(delay) = normalized.strip_prefix("fixed:") {
            let delay_ms = delay
                .parse::<u64>()
                .map_err(|_| AmanError::InvalidRetryBackoff {
        ...
        if let Some(sequence) = normalized.strip_prefix("sequence:") {
            let delays = sequence
                ...
                .map(|part| {
                    part.parse::<u64>().map_err(|_| AmanError::InvalidRetryBackoff {
                        value: value.to_owned(),
                    })
                })
                .collect::<AmanResult<Vec<_>>>()?;

            if delays.is_empty() {
                return Err(AmanError::InvalidRetryBackoff {
                    value: value.to_owned(),
                });
            }
        ...
        Err(AmanError::InvalidRetryBackoff {
            value: value.to_owned(),
        })
    }
}
...
mod tests {
    ...
    fn parses_fixed_backoff() {
        assert_eq!(RetryBackoff::from_str("fixed:250").unwrap(), RetryBackoff::Fixed(250));
    }
