...
Level 4 [98% 满]  → 溢出 AT_LEAST_ONCE 事件到磁盘缓冲区（overflow_to_disk）
Level 5 [100%满]  → 触发紧急模式：停止低优事件源，发系统告警

关键原则：
- AT_MOST_ONCE 事件在任何级别都允许丢，但必须记日志
- AT_LEAST_ONCE 事件在 Level 3 阻塞，Level 4 溢出到磁盘，绝不静默丢弃
...
Pipeline "invoice-processor" {
    ...
    // 或 saga_rollback: false（不做补偿，仅记日志 + 告警）
    // 或 saga_rollback: true（尝试所有补偿，即使部分失败也要继续）

    // Dead Letter Channel（死信通道）
    dlq:
        max_retries: 3                    // 超过 3 次重试仍失败
        on_dlq: alert + log               // 告警 + 记录到 DLQ 存储
        dlq_storage: "/var/lib/agent/dlq/" // DLQ 持久化路径（用于人工接管）
        dlq_ttl_days: 30                  // 在 DLQ 中保留 30 天
}
```
...
任何状态的 ERROR 事件 ──→ ERROR ──→ (7天后自动) ──→ ARCHIVED
```
...
StateStore {
    // 存储隔离
    isolation: namespace | physical   // 命名空间隔离（共享存储，key 前缀区分）
                                      // 物理隔离（独立文件/表/桶）

    // 命名空间规则
    namespace: "skill:{skill_name}"   // 自动添加 skill 名前缀
                                       // Skill A 无法枚举 Skill B 的 key
...
### 决策 8：Pipeline 补偿采用 Saga reverse_order
...
- **代价**：补偿本身可能失败（如已删除的 Slack 消息无法再删除），需要日志记录和人工介入。
- **缓解**：补偿失败不会导致系统崩溃，只会记录告警。补偿本身的执行有超时保护。
...
| Workflow | 内建 State Machine + 超时/错误/归档态 | 无 | 流程驱动 | 链式调用 |
