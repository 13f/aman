...
/// Flatten a YAML metadata value into `HashMap<String, String>`.
///
/// Handles:
/// - `metadata: { triggers: "pdf", tags: "doc" }` → flat strings
/// - `metadata: { hermes: { tags: [RSS], homepage: "..." } }` →
///   `hermes.tags: "RSS"`, `hermes.homepage: "..."` (dot-notation keys)
/// - `metadata: null` or missing → empty map
fn flatten_metadata(value: &YamlValue) -> HashMap<String, String> {
    let mut result = HashMap::new();
    flatten_yaml_value(value, "", &mut result);
    result
}

fn flatten_yaml_value(value: &YamlValue, prefix: &str, out: &mut HashMap<String, String>) {
    match value {
