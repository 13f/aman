...
Standalone mode (--standalone):
  Fetch RSS feeds, store in DB, then run full pipeline:
  search -> score -> summarize -> generate highlights -> output report.
  Reads LLM config from ~/.aman/config.yaml.

The SQLite DB schema mirrors the fusion table. Users manage RSS feeds
externally (e.g. via a cron job that runs --standalone --fetch-only).
"""
...
def main():
    parser = argparse.ArgumentParser(description="RSS DB adapter for info-hub")
    ...
    if args.standalone:
        db_path = args.db_path or os.path.join(Path.home(), ".aman", "rss", "data.db")
    ...
    db_path = input_data.get("db_path", os.path.join(Path.home(), ".aman", "rss", "data.db"))
