...
| Incubation | IdleDetector | **否**（仅 Phase 4.5 关闭时取消。Incubation 是纯后台关联匹配，不因真实事件中断） | 低 | 关联状态保存 → 线程退出 |
...
## 10. Known Risks & Mitigations (R3 Updated)

| 风险 | 来源 | 严重性 | 应对策略 |
|------|------|--------|---------|
| **Depth 重置条件在纯聊天场景下不触发** | **R3-1 P1** | ~~高~~ **已修复** | 条件从 `was_chat && !is_chat` 改为 `was_chat`（包含超时退出和源变更退出） |
| **聊天 Boredom no-op 信息不可达** | **R3-2 P1** | ~~高~~ **已修复** | IdleEvent 新增 `from_chat_mode: bool` 字段；Pipeline 读取此字段决定行为 |
| **Incubation 打断语义不一致** | **R3-3 P2** | **已修复** | 明确 Incubation 为纯后台，不因真实事件中断。仅 Phase 4.5 关闭时取消 |
| **cancel_idle_workflows RwLock 微阻塞** | **R3-4 P2** | **已接受** | 读锁持有时间为纳秒级（Arc clone），无实际风险。未来优化可用 AtomicPtr+CAS |
...
| cancel_idle_workflows RwLock 微阻塞 | R3-4 P2 | 已接受 | 纳秒级，无风险 |
