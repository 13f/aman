...
页面收到 `MESSAGE_ENQUEUED` 事件后，在用户消息气泡上显示队列位置标识（如"消息 #2 排队中"）。页面不强制用户等待，但清晰告知后端状态。
...
- 所有 ChatPlatformSource 产生的 `MESSAGE_RECEIVED` 事件在进入 Event Bus 前先写入 WAL
- Agent 重启后，从 WAL 恢复未消费的事件并重新注入 Event Bus
