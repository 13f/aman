...
//! the [`LlmProvider`](kernel::llm::LlmProvider) trait instead.
...
impl Default for LlmApiProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
impl LlmApiProvider {
    ...
    ///
    /// Retries up to `retries` times with delays of 1s, 2s, 4s, 8s (capped).
...
mod tests {
    ...
    fn parse_with_smart_quotes() {
        // Curly quotes in field names should be replaced
